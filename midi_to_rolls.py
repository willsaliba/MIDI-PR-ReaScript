#for saving a copy of selected midi segement
from sws_python import CF_ExportMediaSource

#to create piano roll from copy of midi segment
import copy
import os
...
def main():
    ...
    #output result
    if numSelected == 1:
        RPR_ShowConsoleMsg("AUDIO SEGMENT SELECTED \n\n")
    else:
        RPR_ShowConsoleMsg("PLEASE SELECT 1 MIDI SEGMENT \nNO ACTION TAKEN \n")
        return

    #check if segment selected is MIDI
    selectedItem = RPR_GetSelectedMediaItem(0, 0)
    take = RPR_GetTake(selectedItem, 0)
    isMIDI = RPR_TakeIsMIDI(take)
    #output result
    if isMIDI:
        RPR_ShowConsoleMsg("SEGMENT IS IN MIDI FORMAT\n\n")
    else:
        RPR_ShowConsoleMsg("SEGMENT IS NOT MIDI FORMAT \nNO ACTION TAKEN \n")
        return

    #save a copy of selected segment !!! set custom path 
    ...
    #output result
    if save_succeded:
        RPR_ShowConsoleMsg("SEGMENT COPY SAVED TO: " + midi_path + "\n\n")
    else:
        RPR_ShowConsoleMsg("SAVING SEGMENT COPY FAILED \nNO ACTION TAKEN \n")
        return
    
    #convert selected MIDI to paino roll
    midi_to_instrument_rolls(save_dir, midi_path)
    RPR_ShowConsoleMsg("INSTRUMENT ROLLS SAVED SUCCESSFULLY\n\n")


#zachs version of jacks code which converts to piano roll
def midi_to_instrument_rolls(midi_dir: str, fname: str):
